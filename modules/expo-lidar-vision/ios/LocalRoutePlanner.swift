...
final class LocalRoutePlanner {
  ...
  func observeRay(
    ...
    if freeSteps > 0 {
      for index in stride(from: 1, through: freeSteps, by: 2) {
  ...
  func guidance(
    ...
    let knownFree = Set(evidence.compactMap { key, value in value.score <= -0.35 ? key : nil })
    guard knownFree.count >= 35 else { return unknown(status: "unknown") }

    let blocked = inflated(obstacles: obstacles, radius: radiusCells)
    ...
    for goal in candidates {
      if let path = findPath(from: start, to: goal, knownFree: knownFree, blocked: blocked),
         bestPath == nil || path.count < bestPath!.count {
        bestPath = path
        bestGoal = goal
      }
    }
    ...
    let confidence = min(1, Double(path.filter { knownFree.contains($0) }.count) / Double(path.count))
  ...
  private func update(_ cell: Cell, delta: Float, person: Bool, timestamp: TimeInterval) {
    ...
    value.score = min(3, max(-2.5, value.score + delta))
  ...
  private func goalCandidates(
    ...
    for longitudinal in stride(from: planningRange, through: 1.2, by: -0.4) {
      for lateral in stride(from: -1.0 as Float, through: 1.0, by: 0.2) {
        ...
        // Prefer forward progress and only then small steering corrections.
        result.append((key, longitudinal * 4 - abs(lateral)))
      }
